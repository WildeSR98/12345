...
while ($true) {
    ...
    try {
        ...
        # Determine best strategies (Top 7)
        $allScores = @()
        foreach ($config in $analytics.Keys) {
            $a = $analytics[$config]
            $score = $a.OK
            if ($a.ContainsKey('PingOK')) { 
                # Bonus for ping stability, small enough to not override actual OK count
                $score += ($a.PingOK / 1000.0) 
            }
            $allScores += [PSCustomObject]@{ Config = $config; Score = $score; OK = $a.OK; PingOK = if ($a.ContainsKey('PingOK')) { $a.PingOK } else { 0 } }
        }
    
        $sortedResults = $allScores | Sort-Object Score -Descending
        $topCount = [Math]::Min($sortedResults.Count, 7)
        $bestConfigs = $sortedResults | Select-Object -First $topCount

        Write-Host ""
        Write-Host "=== TOP $topCount CONFIGURATIONS ===" -ForegroundColor Cyan
        for ($i = 0; $i -lt $bestConfigs.Count; $i++) {
            $c = $bestConfigs[$i]
            Write-Host "  [$($i+1)] $($c.Config.PadRight(35)) (OK: $($c.OK), Ping: $($c.PingOK))" -ForegroundColor Green
        }
    
        $script:installationSuccess = $false
        foreach ($entry in $bestConfigs) {
            Write-Host ""
            Write-Host "============================================================" -ForegroundColor DarkCyan
            Write-Host ">> INSTALLING: $($entry.Config)" -ForegroundColor Yellow
            Write-Host "============================================================" -ForegroundColor DarkCyan
        
            # Restore ipset before starting service (service needs lists)
            if ($originalIpsetStatus -ne "any") {
                Write-Host "[INFO] Restoring ipset lists for service..." -ForegroundColor Gray
                Set-IpsetMode -mode "restore"
            }
        
            # Install via service.cmd
            $serviceBat = Join-Path $rootDir "service.cmd"
            & cmd.exe /c "`"$serviceBat`" install_silent `"$($entry.Config)`""
        
            Write-Host ""
            Write-Host "Service 'zapret' has been installed and started." -ForegroundColor Green
            Write-Host "Please check if YouTube, Discord, etc. are working in your browser." -ForegroundColor Cyan
        
            while ($true) {
                $choice = Read-Host "Does it work? [Y] Yes / [N] Try next / [E] Exit"
                if ($choice -match '^[Yy]$') {
                    Write-Host ""
                    Write-Host "[SUCCESS] Great! Configuration '$($entry.Config)' is now active as a service." -ForegroundColor Green
                    $script:installationSuccess = $true
                    break
                }
                elseif ($choice -match '^[Nn]$') {
                    Write-Host "[INFO] Removing current strategy..." -ForegroundColor Gray
                    & sc.exe stop zapret | Out-Null
                    & sc.exe delete zapret | Out-Null
                    & taskkill /IM winws.exe /F 2>$null | Out-Null
                
                    # Switch back to 'any' for the next step if we need to do anything else, 
                    # but typically we just restore lists each time.
                    break
                }
                ...
                Write-Host "Invalid input. Please enter Y, N, or E." -ForegroundColor Yellow
            }
        
            if ($script:installationSuccess -or ($choice -match '^[Ee]$')) { break }
        }
        ...
        Stop-Zapret
        if (-not $script:installationSuccess) {
            ...
            Write-Host "[INFO] Skipping state restoration to keep the working service active." -ForegroundColor Gray
        }
        Remove-Item -Path $ipsetFlagFile -ErrorAction SilentlyContinue
    }
