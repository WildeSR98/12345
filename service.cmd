@echo off
rem ============================================================================
rem Zapret Service Manager
rem Description: Manages the Zapret DPI bypass service (install, remove, status)
rem Version: 1.9.7b
rem ============================================================================
...
:service_install_silent
cd /d "%~dp0"
call :game_switch_status
set "BIN_PATH=%~dp0bin\"
set "LISTS_PATH=%~dp0lists\"
goto service_install_core
...
set "count=0"
for /f "usebackq delims=" %%F in (`powershell -NoProfile -ExecutionPolicy RemoteSigned -Command "Get-ChildItem -LiteralPath '.' -Filter '*.bat' | Where-Object { $_.Name -notlike 'service*' } | Sort-Object { [Regex]::Replace($_.Name, '(\d+)', { $args[0].Value.PadLeft(8, '0') }) } | ForEach-Object { $_.Name }"`) do (
    set /a count+=1
    echo !count!. %%F
    set "file!count!=%%F"
)
...
pause
if defined selectedFile exit /b
goto menu
...
set "lineCount=0"
if exist "%listFile%" for /f %%i in ('find /c /v "" ^< "%listFile%"') do set "lineCount=%%i"
...
echo.
start "" powershell -NoProfile -ExecutionPolicy RemoteSigned -File "%~dp0utils\test zapret.ps1"
pause
goto menu
